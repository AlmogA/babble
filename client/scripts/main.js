...
function poll() {
    ...
    // almost every 10 minutes the request dropping, and we send new request
    Babble.lastTimeout = setTimeout(function(){xhr.abort(); poll();}, 590000);
    xhr.send(); 
}
...
function statusPoll() {
    ...
    // new request every almost 10 minutes
    Babble.lastTimeoutStatus = setTimeout(function(){xhr.abort(); statusPoll();}, 590000);
    xhr.send(); 
}
